...
const createReplyContext = (title, body) => ({
    contextInfo: {
        isForwarded: true,
        forwardingScore: 999,
        forwardedNewsletterMessageInfo: {
            newsletterName: global.botName,
            newsletterJid: global.Jid,
            serverMessageId: -1
        },
        externalAdReply: {
            title: title,
            body: body,
            thumbnail: fs.existsSync(global.thumbLocal) ? fs.readFileSync(global.thumbLocal) : global.thumb,
            mediaType: 1,
...
const handleAntiLink = async (sock, m) => {
    try {
        ...
        if (mode === 'delete') {
            await sock.sendMessage(from, { text: infoText, ...createReplyContext(tittle, global.botName) })
        } else if (mode === 'remove') {
            await sock.sendMessage(from, { text: infoText, ...createReplyContext(tittle, global.botName) })
            try {
                await sock.groupParticipantsUpdate(from, [sender], 'remove')
            } catch (e) {                await sock.sendMessage(from, { text: `Could not remove ${pushname} from group. Bot may not be admin.`, ...createReplyContext(global.tittle, global.botName) })
            }
        }
    } catch (e) {
    ...
    document: 'Document', contact: 'Contact', location: 'Location'
        ...
        if (mode === 'delete') {
            ...
            try {
                ...
                await sock.sendMessage(from, { text: `Could not remove ${pushname} from group. Bot may not be admin.`, ...createReplyContext(global.tittle, global.botName) })
            }
        }
    } catch (e) {
...
const handleAntiDelete = async (sock, deletedKey) => {
    try {
        ...
        if (msgType === 'conversation') {
            await sock.sendMessage(sendTo, { text: `${info}\n\n💬 *Message :* ${inner}`, ...createReplyContext(global.tittle, global.botName) })
        } else if (msgType === 'extendedTextMessage') {
            await sock.sendMessage(sendTo, { text: `${info}\n\n💬 *Message :* ${inner?.text || ''}`, ...createReplyContext(global.tittle, global.botName) })        } else if (['imageMessage', 'videoMessage', 'audioMessage', 'documentMessage', 'stickerMessage'].includes(msgType)) {
            const fakeMsg = {
            ...
            if (msgType === 'imageMessage') {
                await sock.sendMessage(sendTo, { image: buffer, caption, ...createReplyContext(global.tittle, global.botName) })
            } else if (msgType === 'videoMessage') {
                await sock.sendMessage(sendTo, { video: buffer, caption, mimetype: inner?.mimetype || 'video/mp4', ...createReplyContext(global.tittle, global.botName) })
            } else if (msgType === 'audioMessage') {
                await sock.sendMessage(sendTo, { audio: buffer, mimetype: inner?.mimetype || 'audio/mp4', ptt: inner?.ptt || false, ...createReplyContext(global.tittle, global.botName) })
            } else if (msgType === 'stickerMessage') {
                await sock.sendMessage(sendTo, { sticker: buffer, ...createReplyContext(global.tittle, global.botName) })
            } else {
                await sock.sendMessage(sendTo, { document: buffer, mimetype: inner?.mimetype || 'application/octet-stream', fileName: inner?.fileName || 'file', caption, ...createReplyContext(global.tittle, global.botName) })
            }
        }
    } catch (e) {
...
const handleGroupUpdates = async (sock, update) => {
    try {
        ...
        for (const num of participants) {
            ...
            await sock.sendMessage(BotNum, { text: reportText, ...createReplyContext(global.tittle, global.botName) })
        }
...
module.exports = async function SarDevHandler(sock, m, chatUpdate, store) {
    try {
        ...
        const ReplySuccess = (text) => sock.sendMessage(from, {
            text: `${text}`,
            ...createReplyContext(tittle, global.botName)
        }, { quoted: m })
        ...
        const ReplySar = (text) => sock.sendMessage(from, {
            text: `DATABASE EMPTY\n\n${text}`,
            ...createReplyContext(global.tittle, global.botName)
        }, { quoted: m })
...
┗━━━ *Powered by ${global.botName}* ━━━┛`
                const menuContext = createReplyContext(global.tittle, global.botName)
menuContext.contextInfo.mentionedJid = [sender]

await sock.sendMessage(from, {
    text: teks,
    contextInfo: menuContext.contextInfo
}, { quoted: m })
            ...
            case 'adddatabase': {
                ...
                const db = getLocalDb()
                if (db.includes(nomor)) return ReplyFailed('Number already exists in database')
            ...
            case 'listdatabase': {
                ...
                const db = getLocalDb()
                if (db.length === 0) return ReplySar('Database is empty')
